...
//
package com.amlogic.mediacenter.airplay.player;
...
public class BlobCache implements Closeable {
	...
	private boolean loadIndex() {
		try {
			...
			if (mIndexFile.read(buf) != INDEX_HEADER_SIZE) {
				Debug.w(TAG, "cannot read header");
				return false;
			}

			if (readInt(buf, IH_MAGIC) != MAGIC_INDEX_FILE) {
				Debug.w(TAG, "cannot read header magic");
				return false;
			}

			if (readInt(buf, IH_VERSION) != mVersion) {
				Debug.w(TAG, "version mismatch");
				return false;
			}
			...
			if (checkSum(buf, 0, IH_CHECKSUM) != sum) {
				Debug.w(TAG, "header checksum does not match");
				return false;
			}

			// Sanity check
			if (mMaxEntries <= 0) {
				Debug.w(TAG, "invalid max entries");
				return false;
			}
			if (mMaxBytes <= 0) {
				Debug.w(TAG, "invalid max bytes");
				return false;
			}
			if (mActiveRegion != 0 && mActiveRegion != 1) {
				Debug.w(TAG, "invalid active region");
				return false;
			}
			if (mActiveEntries < 0 || mActiveEntries > mMaxEntries) {
				Debug.w(TAG, "invalid active entries");
				return false;
			}
			if (mActiveBytes < DATA_HEADER_SIZE || mActiveBytes > mMaxBytes) {
				Debug.w(TAG, "invalid active bytes");
				return false;
			}
			if (mIndexFile.length() != INDEX_HEADER_SIZE + mMaxEntries * 12 * 2) {
				Debug.w(TAG, "invalid index file length");
				return false;
			}
			...
			if (mDataFile0.read(magic) != 4) {
				Debug.w(TAG, "cannot read data file magic");
				return false;
			}
			if (readInt(magic, 0) != MAGIC_DATA_FILE) {
				Debug.w(TAG, "invalid data file magic");
				return false;
			}
			...
			Debug.e(TAG, "loadIndex failed.", ex);
			return false;
		}
	}
	...
	private boolean getBlob(RandomAccessFile file, int offset, LookupRequest req)
		...
		try {
			...
			if (file.read(header) != BLOB_HEADER_SIZE) {
				Debug.w(TAG, "cannot read blob header");
				return false;
			}
			...
			if (file.read(blob, 0, length) != length) {
				Debug.w(TAG, "cannot read blob data");
				return false;
			}
			if (checkSum(blob, 0, length) != sum) {
				Debug.w(TAG, "blob checksum does not match: " + sum);
				return false;
			}
			return true;
		} catch (Throwable t) {
			Debug.e(TAG, "getBlob failed.", t);
			return false;
		} finally {
	...
	private boolean lookupInternal(long key, int hashStart) {
		...
		while (true) {
			...
			if (candidateOffset == 0) {
				...
				if (slot == slotBegin) {
					Debug.w(TAG, "corrupted index: clear the slot.");
	...
	public void syncIndex() {
		try {
			...
			Debug.w(TAG, "sync index failed", t);
		}
	}
	...
	public void syncAll() {
		...
		try {
			...
			Debug.w(TAG, "sync data file 0 failed", t);
		}
		try {
			mDataFile1.getFD().sync();
		} catch (Throwable t) {
			Debug.w(TAG, "sync data file 1 failed", t);
		}
	}
	...
	int getActiveCount() {
		...
		if (count == mActiveEntries) {
			...
			Debug.e(TAG, "wrong active count: " + mActiveEntries + " vs " + count);
